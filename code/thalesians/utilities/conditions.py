...
import thalesians.settings as settings

def conditions(pre=None, post=None, message='Condition violated', level=1):
    def decorator(func):
        # Use functools to preserve the name, docstring, etc.
        if settings.MIN_PRECONDITION_LEVEL <= level and settings.MIN_POSTCONDITION_LEVEL <= level:
            @functools.wraps(func)
            def wrapper(*args, **kwargs):  # NB: No self
                if pre is not None:
                    assert pre(*args, **kwargs), message
                retval = func(*args, **kwargs)
                if post is not None:
                    assert post(retval), message
                return retval
        elif settings.MIN_PRECONDITION_LEVEL <= level:
            @functools.wraps(func)
            def wrapper(*args, **kwargs):  # NB: No self
                if pre is not None:
                    assert pre(*args, **kwargs), message
                return func(*args, **kwargs)
        elif settings.MIN_POSTCONDITION_LEVEL <= level:
            @functools.wraps(func)
            def wrapper(*args, **kwargs):  # NB: No self
                retval = func(*args, **kwargs)
                if post is not None:
