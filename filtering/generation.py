...
class StateSpaceModelDataGenerator(object):
    class Data(object):
        ...
        def noisedf(self, columnname=lambda idx, name: name):
            from pandas import DataFrame
            series = OrderedDict()
            for idx, name in enumerate(self._noisenames):
                series[columnname(idx, name)] = self._noises[:,idx]
            return DataFrame(series, index=self.times())
        
        def jumpdf(self, columnname=lambda idx, name: name):
            from pandas import DataFrame
            series = OrderedDict()
            for idx, name in enumerate(self._jumpnames):
                series[columnname(idx, name)] = self._jumps[:,idx]
            return DataFrame(series, index=self.times())
        
        def jumpflagdf(self, columnname=lambda idx, name: name):
            from pandas import DataFrame
            series = OrderedDict()
            for idx, name in enumerate(self._jumpnames):
                series[columnname(idx, name)] = self._jumpflags[:,idx]
            return DataFrame(series, index=self.times())
        
        def processdf(self, columnname=lambda idx, name: name):
            from pandas import DataFrame
            series = OrderedDict()
            for idx, name in enumerate(self._processnames):
                series[columnname(idx, name)] = self._processes[:,idx]        
            return DataFrame(series, index=self.times())
    ...
    def setnoisecors(self, cors):
        cors = SubdiagonalArray.create(cors)
    ...
    def __generatenoises(self):        
        ...
        covmatrix = thalesians.maths.stats.cor2cov(self.__noisecors, var=self.__noisevars)
